...
function MobileFrame({ header, bottomNav, children, contentBg = 'white' }) {
  ...
  return (
    <div className="bg-white md:bg-[#F4F4F5] w-full min-h-[100dvh] flex justify-center">
      <div className={`relative ${innerBg} w-full flex flex-col min-h-[100dvh] mx-auto md:max-w-[480px] md:rounded-[24px] md:shadow-2xl md:my-8`}>
        {header}
        <div className={`flex flex-col flex-1 ${bottomNav ? 'pb-24' : ''}`}>{children}</div>
        {bottomNav}
      </div>
    </div>
  )
