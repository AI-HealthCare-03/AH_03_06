  ...
  return (
    <MobileFrame header={<Header variant="back" title="내 포인트" onBack={() => navigate(-1)} />} bottomNav={<BottomNav />} contentBg="white">
      <div className="px-5 pt-5 space-y-4">
        ...
        {history && history.length === 0 && (
          /* 빈 상태 — 공용 EmptyState 톤 통일 (CTA는 action으로 유지) */
          <EmptyState
            icon={faStar}
            title="아직 적립 내역이 없어요"
            description="출석체크로 첫 포인트를 받아보세요"
            className="pt-16"
            action={
              <button onClick={() => { resetAttendanceCheckSession(); navigate('/home') }}
                className="px-7 h-12 bg-primary text-white text-[15px] font-bold rounded-xl active:opacity-80">
                출석체크 하러 가기
              </button>
            }
          />
