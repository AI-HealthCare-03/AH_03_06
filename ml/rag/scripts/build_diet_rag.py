"""식단 가이드 PDF를 ChromaDB 컬렉션 ``diet_guidelines``로 구축한다."""
...
DOC_DIR = ROOT / "rag" / "documents" / "diet"
PERSIST_DIR = ROOT / "rag" / "embeddings" / "chroma_db"
COLLECTION_NAME = "diet_guidelines"
...
def _chunk_text(text: str, max_chars: int = 900) -> list[str]:
    """문단 단위로 나눈 뒤, 긴 문단은 문장 단위로 추가 분할한다."""
...
def build_diet_vectorstore() -> None:
    """``rag/documents/diet/`` 내 PDF를 로드해 임베딩 후 ChromaDB에 저장한다."""
