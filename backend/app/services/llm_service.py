...
## 출력 필드
- key_point: 지금 가장 중요한 포인트, 2문장 이내. 아래 sections 가 직접 뒷받침하는 내용만.
- sections[]: { title, scope, gloss, quote_raw, quote_display, source }
  - title: 발췌 주제를 짧게(8자 안팎). 범위 일반화 금지.
  - scope: 적용 대상/상황. 예 "전체" / "간기능부전 환자" / "투여 중지 후 다른 혈압강하제 사용 시".
...
6. 행동지시(용량 변경·중단·약 교체 직접 지시) 금지 — 상담 권유로.
7. 사용자가 해당 환자군인지 알 수 없는 소집단 한정 주의는 key_point 에 넣지 않는다(sections.scope 로만).

## 섹션 선택
- sections 는 검색된 관련 발췌만큼 1~4개. 억지로 채우지 않는다. 질문(있으면)에 직접 답하는 발췌 우선.
...
- scope 가 발췌의 적용 범위와 일치하는가? JSON 외 텍스트가 없는가?
"""
